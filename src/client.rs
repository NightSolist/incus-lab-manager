...
impl IncusClient {
    ...
    async fn wait_for_operation(&self, resp: reqwest::Response) -> Result<()> {
        if !resp.status().is_success() {
            bail!("Request failed: {:?}", resp.text().await?);
        }
    ...
    pub async fn create_profile(&self, req: &ProfilesPost) -> Result<()> {
        let url = format!("{}/1.0/profiles", self.base_url);
        let resp = self.http.post(&url).json(req).send().await?;
        self.wait_for_operation(resp).await
    }

    pub async fn update_profile(&self, name: &str, req: &ProfilePut) -> Result<()> {
        let url = format!("{}/1.0/profiles/{}", self.base_url, name);
        let resp = self.http.put(&url).json(req).send().await?;
        self.wait_for_operation(resp).await
    }
    ...
    pub async fn create_storage_pool(&self, req: &StoragePoolsPost) -> Result<()> {
        let url = format!("{}/1.0/storage-pools", self.base_url);
        let resp = self.http.post(&url).json(req).send().await?;
        self.wait_for_operation(resp).await
    }

    pub async fn update_storage_pool(&self, name: &str, req: &StoragePoolPut) -> Result<()> {
        let url = format!("{}/1.0/storage-pools/{}", self.base_url, name);
        let resp = self.http.put(&url).json(req).send().await?;
        self.wait_for_operation(resp).await
    }
